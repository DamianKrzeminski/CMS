...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(strlen($username)<4){
        $error['username'] = 'Username needs to have more than 4 characters';
    }
    if($username == ''){
        $error['username'] = 'Username cannot be empty';
    }
    if(username_exists($username)){
        $error['username'] = 'Username already exists, chose another one';
    }
    if($email == ''){
        $error['email'] = 'Email cannot be empty';
    }
    if(email_exists($email)){
        $error['email'] = 'Email already exists, chose another one';
    }
    if($password == ''){
